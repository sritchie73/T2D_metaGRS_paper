...
system("mkdir -p family_history")
fwrite(info, file="family_history/field_information.csv")
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
    file=sprintf("field_lists/family_history_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/family_history_fields_* --destination 'common/Family History/scripts/'", wait=TRUE)