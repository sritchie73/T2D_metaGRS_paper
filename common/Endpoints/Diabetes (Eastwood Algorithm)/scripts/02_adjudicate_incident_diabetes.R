...
system("mkdir -p eastwood_diabetes") 

########################################################
# Download data we need for output
########################################################

system("mkdir -p input_data")

download <- function(fpath) {
  if (!file.exists(sprintf("input_data/%s", basename(fpath)))) {
    system(sprintf("dx download '%s' -o input_data/", fpath))
  }
}
download("common/Demographics/demographics.csv")
download("common/Hospital Records/diagnoses.csv")
download("common/Deaths/death_causes.csv")
download("common/Follow-up/follow_up.csv")
...
hes <- fread("input_data/diagnoses.csv")
deaths <- fread("input_data/death_causes.csv")
followup <- fread("input_data/follow_up.csv")

# Load assessment visit information
assessments <- fread("input_data/demographics.csv")
...
system("rm -f eastwood_diabetes/incident_diabetes_adjudication.log")
cat <- function(...) {
  base::cat(..., file="eastwood_diabetes/incident_diabetes_adjudication.log", append=TRUE)
}
...
events <- rbind(events, unique(hes[, .(eid, event="Hospital episode", 
  start_date=event_start, end_date=event_end)]))
...
# available
events <- rbind(events, unique(followup[(has_died),
  .(eid, event="Death", start_date=max_death_censor, 
    end_date=max_death_censor)]))

events <- rbind(events, unique(followup[max_death_location == "Lost", 
  .(eid, event="Lost to followup", start_date=max_death_censor,
    end_date=max_death_censor)]))

events <- rbind(events, unique(followup[, .(eid, 
    event="Min hospital followup", start_date=min_hospital_censor,
    end_date=min_hospital_censor)]))

events <- rbind(events, unique(followup[!(has_died) & max_hospital_location != "Lost",
  .(eid, event="Max hospital followup", start_date=max_hospital_censor,
    end_date=max_hospital_censor)]))

events <- rbind(events, unique(followup[!(has_died) & max_death_location != "Lost",
  .(eid, event="Max deaths followup", start_date=max_death_censor,
    end_date=max_death_censor)]))
...
hes_fix[, icd_version := 10]
hes[hes_fix, on = .(eid, icd_version, event_start=start_date), event_end := event_start]
...
hes_known_cause <- hes[!is.na(diag_icd), .SD[1], 
  by=.(eid, start_date=event_start, end_date=event_end)]
hes_known_cause[, event := "Hospital episode"]
events[hes_known_cause, on = .(eid, event), c("t1d", "t2d", "diab_ns") := FALSE]

hes_t1d <- hes[icd_version == 10 & diag_icd %starts_with% "E10", 
  .(eid, event="Hospital episode", start_date=event_start, end_date=event_end)]
events[hes_t1d, on = .(eid, event, start_date, end_date), t1d := TRUE]

hes_t2d <- hes[icd_version == 10 & diag_icd %starts_with% "E11", 
  .(eid, event="Hospital episode", start_date=event_start, end_date=event_end)]
events[hes_t2d, on = .(eid, event, start_date, end_date), t2d := TRUE]

hes_diab_ns <- hes[icd_version == 10 & diag_icd %starts_with% c("E13", "E14"), 
  .(eid, event="Hospital episode", start_date=event_start, end_date=event_end)]
...
diab_followup <- cbind(assessments,
...
diab_followup[!followup[(has_died)], on = .(eid), diab_death_record := FALSE]
...
prev_diab <- fread("eastwood_diabetes/prevalent_diabetes.csv")
...
linkage_withdrawn <- dat[!(eid %in% followup$eid)]
...
exited_1.3[followup[max_hospital_location == "Lost"], on = .(eid, assessment_date < max_hospital_censor), adjudication_note := paste(adjudication_note,
  sprintf("1.3: Lost to follow-up prior to UK Biobank assessment visit with reason \"%s\" and no evidence of prevalent diabetes.", max_hospital_reason))] 
exited_1.3[followup[(has_died)], on = .(eid), adjudication_note := paste(adjudication_note, 
  "1.3: Died from non-diabetes related causes, no diabetes in health records.")]
exited_1.3[followup[max_hospital_location == "Lost"], on = .(eid, assessment_date<=max_hospital_censor), adjudication_note := paste(adjudication_note,
  sprintf("1.3: No diabetes until lost to follow-up with reason \"%s\".", max_hospital_reason))]
exited_1.3[followup[!(has_died) & max_hospital_location != "Lost"], on = .(eid), adjudication_note := paste(adjudication_note,
  sprintf("1.3: Alive at maximum follow-up date (%s for hospitals in %s) with no diabetes in health records.",
    max_hospital_censor, max_hospital_location))] 
...
fwrite(final, file="eastwood_diabetes/incident_diabetes.csv")
fwrite(diab_followup, file="eastwood_diabetes/incident_diabetes_extended_info.csv")

# Upload to persistent storage
system("dx upload eastwood_diabetes/incident_diabetes.csv --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
system("dx upload eastwood_diabetes/incident_diabetes_extended_info.csv --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
system("dx upload eastwood_diabetes/incident_diabetes_adjudication.log --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
