...
download("common/Medical History/medical_history.csv")
download("common/Medications/touchscreen_survey_medications.csv")
download("common/Medications/verbal_interview_medications.csv")
...
demo <- fread("input_data/demographics.csv")
dat <- demo[, .(eid, visit_index, sex)]
...
eth <- demo[, .(eid, visit_index, ethnicity_SAorAC=fcase(
...
# load self-report medical history
self_report <- fread("input_data/medical_history.csv")
self_report[(imputed), c("age", "date") := NA] # ignore custom imputation of missing event dates
...
meds_ts <- fread("input_data/touchscreen_survey_medications.csv")
...
meds_ni <- fread("input_data/verbal_interview_medications.csv")
...
system("rm -f eastwood_diabetes/prevalent_diabetes_adjudication.log")
cat <- function(...) {
  base::cat(..., file="eastwood_diabetes/prevalent_diabetes_adjudication.log", append=TRUE)
}
...
# Curate age of diagnosis for gestational diabetes
exited_1.2[,age_diab := age_diab_ts]
exited_1.2[!is.na(age_gdm_ni), age_diab := age_gdm_ni]
...
final <- demo[,.(eid, visit_index)]
dat2 <- dat[, .(eid, visit_index, adjudicated_diabetes, adjudication_finalized, adjudication_note, age_of_onset=age_diab)]
final <- dat2[final, on = .(eid, visit_index)]

fwrite(final, sep="\t", quote=FALSE, file="eastwood_diabetes/prevalent_diabetes.csv")

# Also extract diabetes medication column
meds <- dat[,.(eid, visit_index, 
  insulin_ni, insulin_ts, insulin_1yr_diab_ts,
  metformin_ni, other_diab_med_ni
)]
fwrite(meds, sep="\t", quote=FALSE, file="eastwood_diabetes/diabetes_medications.csv")
  
# Upload to persistent storage
system("dx upload eastwood_diabetes/prevalent_diabetes.csv --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
system("dx upload eastwood_diabetes/diabetes_medications.csv --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
system("dx upload eastwood_diabetes/prevalent_diabetes_adjudication.log --destination 'common/Endpoints/Diabetes (Eastwood Algorithm)/'")
