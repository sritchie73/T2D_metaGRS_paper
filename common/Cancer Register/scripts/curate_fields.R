...
fields <- data_dict[folder_path %like% "Cancer register"]
fields <- rbind(data_dict[name == "eid" & entity == "participant"], fields)
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
         file=sprintf("field_lists/cancer_register_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/cancer_register_fields_* --destination 'common/Cancer Register/scripts/'", wait=TRUE)
