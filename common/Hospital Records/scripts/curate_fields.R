...
fields <- data_dict[entity %in% c("hesin", "hesin_oper", "hesin_diag")]
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
         file=sprintf("field_lists/hospital_records_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/hospital_records_fields_* --destination 'common/Hospital Records/scripts/'", wait=TRUE)
