...
info <- rbind(use.names=FALSE,
  data.table(field.id=21002, name="Weight (Kg)", var="weight"),
  data.table(field.id=50, name="Standing height (cm)", var="height"),
  data.table(field.id=21001, name="Body Mass Index (Kg/m^2)", var="bmi"),
  data.table(field.id=48, name="Waist circumference (cm)", var="waist"),
  data.table(field.id=49, name="Hip circumference (cm)", var="hip")
)

# Save table of information
system("mkdir -p Anthropometrics")
fwrite(info, file="Anthropometrics/field_information.csv")
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
    file=sprintf("field_lists/anthropometrics_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/anthropometrics_fields_* --destination 'common/Anthropometrics/scripts/'", wait=TRUE)