...
info <- rbind(use.names=FALSE,
  data.table(field.id=23165, name="Blood-type haplotype", var="blood_type"),
  data.table(field.id=30079, name="Genomic ancestry", var="ancestry")
)

# Save table of information
system("mkdir -p genetic_reference")
fwrite(info, file="genetic_reference/field_information.csv")
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
         file=sprintf("field_lists/genetic_reference_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/genetic_reference_fields_* --destination 'common/Genetic Reference/scripts/'", wait=TRUE)
