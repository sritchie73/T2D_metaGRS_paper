...
fields <- data_dict[entity %like% "gp_"]

# Write out field IDs for each distinct entity
...
for (entity_type in unique(fields$entity)) {
  fwrite(fields[entity == entity_type, .(name)], quote=FALSE, col.names=FALSE,
    file=sprintf("field_lists/primary_care_fields_%s_entity.txt", entity_type))
}

# Upload field list to persistent storage
system("dx upload field_lists/primary_care_fields_* --destination 'common/Primary Care/scripts/'", wait=TRUE)