...
g <- ggplot(eCDF_check) +
  ...
  scale_x_continuous("Predicted 10-year T2D risk", labels=scales::percent, limits=c(0,0.3), oob=oob_censor, expand=expansion(mult=0, add=c(0.01, 0.03))) +
...
nri.test <- function(data, qdiab_model, risk_threshold) {
  ...
  comp_risk[, model := ifelse(model == "QDiabetes", "base", "new")]
  comp_risk <- dcast(comp_risk, eid + incident_censor_years + incident_type_2_diabetes ~ model, value.var="absrisk")
...
nri_lists <- list(
  "5%"=list(
     "QDiabetes model A"=nri.test(eCDF_check, "QDiabetes model A", 0.05),
     "QDiabetes model B"=nri.test(eCDF_check, "QDiabetes model B", 0.05),
     "QDiabetes model C"=nri.test(eCDF_check, "QDiabetes model C", 0.05)
  ),
  "10%"=list(
     "QDiabetes model A"=nri.test(eCDF_check, "QDiabetes model A", 0.1),
     "QDiabetes model B"=nri.test(eCDF_check, "QDiabetes model B", 0.1),
     "QDiabetes model C"=nri.test(eCDF_check, "QDiabetes model C", 0.1)
  ),
  "15%"=list(
     "QDiabetes model A"=nri.test(eCDF_check, "QDiabetes model A", 0.15),
     "QDiabetes model B"=nri.test(eCDF_check, "QDiabetes model B", 0.15),
     "QDiabetes model C"=nri.test(eCDF_check, "QDiabetes model C", 0.15)
  )
)
saveRDS(nri_lists, file="output/UKB_tests/categorical_nri.rds")
...
fwrite(nri_estimates, sep="\t", quote=FALSE, file="output/UKB_tests/categorical_nri_estimates.txt")
fwrite(reclassified, sep="\t", quote=FALSE, file="output/UKB_tests/categorical_nri_reclassified.txt")
