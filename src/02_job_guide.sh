...
# (3) Filters to alleles with INFO > 0.4 in UKB.
...
# Extracts from the pre-collated UKB phenotype data (see ext_src/) the training set 
# of participants to use for LDpred2 hyperparameter tuning.
#
# This training set comprises the ~12K participants used to train the previously 
# published Stroke metaGRS, with additional filters applied:
#
# (1) Participants with suspected T2D at baseline or suspected pre-diabetes/impaired
#     glucose control are excluded to maximise separation between cases (adjudicated 
#     prevalent T2D using the Eastwood et al. algorithm) and controls.
# (2) First- and second- degree relatives are removed, for each pair prioritising 
#     T2D cases.
#
# Data is written out to data/ldpred2/training_sample_phenotypes.txt
sbatch -c 1 --time 0:10:0 --wrap "Rscript --vanilla ./src/02_GRS_train/03_extract_pheno.R"
...
sbatch -c 22 --time 2:0:0 --wrap "Rscript --vanilla ./src/02_GRS_train/04_extract_geno.R"

# Compute SNP-SNP correlation matrices on each chromosome to be used by LDpred2.
sbatch --mem 80000 --time 2:0:0 --wrap "Rscript --vanilla ./src/02_GRS_train/05_compute_genetic_corr.R"
...
sbatch --array=1-44 --mem 160000 --time 12:0:0 --partition skylake,skylake-himem \
  --wrap "Rscript --vanilla ./src/02_GRS_train/06_run_ldpred.R"

sbatch --array=8,9,18,27,28,29,43 --mem 160000 --time 12:0:0 --partition skylake,skylake-himem \
  --wrap "Rscript --vanilla ./src/02_GRS_train/06_run_ldpred.R"
