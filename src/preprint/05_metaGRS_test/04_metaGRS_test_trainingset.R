...
system("mkdir -p output/metaGRS/test/trainingset/trainingset", wait=TRUE)
...
fwrite(cc, sep="\t", quote=FALSE, file="output/metaGRS/test/trainingset/cohort_case_numbers.txt")
...
for (mIdx in case_control_definitions[,.I]) {
  ...
  fwrite(perf, sep="\t", quote=FALSE, file=sprintf("output/metaGRS/test/trainingset/%s_model_performance.txt", this_model$short_name))
}
