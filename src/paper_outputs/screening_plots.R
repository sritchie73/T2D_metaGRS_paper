...
screening <- fread("output/UKB_tests/screening_nri_estimates.txt")
screening[, risk_threshold := factor(risk_threshold, levels=c("15%", "14.3%", "13.2%", "10%", "5.6%"))]
...
g <- ggplot(screening[risk_threshold %in% c("5.6%", "10%", "15%")]) +
...
ggsave(g, width=3.3, height=1.6, file="output/paper_outputs/figures/screening_nri.pdf")
...
strata_num <- fread("output/UKB_tests/screening_nri_reclassified.txt")
...
# Write out
fwrite(supp_cases, sep="\t", quote=FALSE, file="output/paper_outputs/tables/screening_nri_cases.txt")
fwrite(supp_controls, sep="\t", quote=FALSE, file="output/paper_outputs/tables/screening_nri_controls.txt")
